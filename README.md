...
## IMPORTANT: Major changes in V3.0.0:
   ...
   * As of May 2017, you should use the **executable "bandup"** for 
     **all BandUP tasks**. This is a **Python** script that provides
     ...
     executed  directly in previous versions. The new "bandup" script
     also provides a "plot" task which reimplements the previously 
     available plotting tool and its GUI -- both of which no
   ...
   * The individual fortran executables maintain back-compatibility,
     ...
     The tutorials have been updated though, and **new bash scripts**
     have been provided. These are much simpler than their previous 
...
## Tips (some of which already mentioned above):
   ...
   * **It might be handy placing a symlink to the "bandup" executable
     in some directory that belongs to your PATH**. Alternatively
     (but less desirably), you can include the current (directory in
     your PATH is you prefer to do so. In either case, you'll then be
     able to use BandUP in making directory you're working at.
