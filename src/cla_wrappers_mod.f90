...
!! @param[in] running_from_main_code Should be .TRUE. if running from the main code
!=====================================================================================
...
logical :: main_code
    ...
    call cla_register(key='-n_sckpts_to_skip', &
                      description='Tells BandUP to skip the first "n_sckpts_to_skip" &
                                   ...
                                   '     calcs. using VASP.', &
                      kkind=cla_int, default='0')
    call cla_register(key='-continue_if_not_commensurate', &
                      description='Continue if the SC and pc are not commensurate.', &
                      kkind=cla_flag, default='F')
    call cla_register(key='-skip_propose_pc_for_given_pc', &
                      description='Do not attempt to propose a pc for the input &
                                   reference unit cell.', &
                      kkind=cla_flag, default='F')
    call cla_register(key='-skip_propose_pc_for_given_sc', &
    ...
    call cla_register(key='-normal_to_proj_plane', &
                      description='Applies only to noncollinear calculations. &
                                   ...
                                   //new_line('A'), &
                      kkind=cla_char, default='0, 0, 1')
    call cla_register(key='-origin_for_spin_proj_rec', &
                      description='', &
                      kkind=cla_char, default='0, 0, 0')
    call cla_register(key='-origin_for_spin_proj_cart', &
                      description='', &
                      kkind=cla_char, default='0, 0, 0')
    call cla_register(key='--continue_if_npw_smaller_than_expected', &
                      description='Ignore error if the estimated npws are smaller than &
                       the ones found.', &
                      kkind=cla_flag, default='F')
    call cla_register(key='-dont_unfold', &
