...
!! has been granted by the Copyright holders.
...
!!$*
!!$*       3: Some small rorganization of the code that are not so important, but are worth mentioning
!!$*          3.a: The size of the WAVECAR file is now passed to the user (optional)
!!$*          3.b: The constant "c" (vasp_2m_over_hbar_sqrd) is now defined by means of a "parameter" statement 
!!$*               instead of the original "data"
!!$*          3.c: Replaced uni=6 by unit=* in the write statements
!!$*          3.d: Pi is now declared as a parameter, and a parameter "twopi" has now been defined as well.
!!$*          3.e: The names of some variables have been changed (just because of my personal style of writing)
!!$*
...
if(.not.present(i_selected_kpt))then
    if(present(n_plane_waves)          .or. &
       ...
       present(coeff)                 )then
        write(*,'(A)')'ERROR (read_wavecar_mod): Not able to retrieve information from WAVECAR. Please specify a k-point index.'
...
if(present(spin_channel))then
    if((spin_channel>=1).and.(spin_channel<=nspin))then
        ...
        write(*,'(A)')'WARNING (read_wavecar_mod): Reading information for the first spin channel.'
    endif
endif
...
!   E(K+G)<=ENCUT
    ...
    if (ncnt.ne.nplane) then
        write(*,'(A,I0,A)') 'ERROR in wave vector K(',i_selected_kpt,'):' 
        write(*,'(2(A,I0))') 'Calculated number of K-vectors Km = K(',i_selected_kpt,') + Gm for which E(Km) < ENCUT (Gm is a rec. latt. vec.): ',ncnt
        write(*,'(A,I0)') 'Number of such vectors found in the input file for the same K-point: ',nplane
        write(*,'(A)') 'The two numbers differ. Stopping now.'
        stop
    endif
    if (ncnt.gt.npmax) then
        write(*,'(A,I0,A)') 'ERROR in wave vector K(',i_selected_kpt,'):' 
        write(*,'(2(A,I0))') 'Calculated number of K-vectors Km = K(',i_selected_kpt,') + Gm for which E(Km) < ENCUT (Gm is a rec. latt. vec.): ',ncnt
        write(*,'(A,I0)') 'Estimated maximum number of such vectors: ',npmax
        write(*,'(A)') 'Stopping now.'
        stop
    endif
  
    if(present(i_allg_in_file))then
        allocate (i_allg_in_file(3,npmax))
        i_allg_in_file(:,:) = igall(:,:)
    endif

    if(present(coeff))then
        allocate (coeff(npmax,nband))

        ! The variables "available_io_units", "irec_before_loop" and "temp_unit" have been added to make it possible to use openmp 
        ! to parallelize (over bands) the reading of the WAVECAR file.
        ...
        !$    shared(nband,irec_before_loop,available_io_units,coeff,nplane,nrecl)
        do iband=1,nband
