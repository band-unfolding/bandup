...
print 'Max. delta_N:          ',np.max(delta_Ns)
print 'Min. non-zero delta_N: ',np.min(delta_Ns[np.nonzero(delta_Ns)])
...
print 'Done with eliminating duplicated points.'
...
if xmin>KptsCoords.min() or xmax<KptsCoords.max() or ymin>energies.min() or ymax<energies.max():
    print "Reducing data so that only the nedded part of it is parsed."
    new_KptsCoords = []
    new_energies = []
    new_delta_Ns = []
...
print "Working with ","%.*f" % (2, 100.0*size_of_new_data/size_of_old_data),"% of the data read in."
...
for idir in range(ndirections):
    if(coords_type[0].upper() == 'R'):
        ...
        if((coords_type[0].upper() != 'C') and (idir==0)):
            print 'Assuming that the pc-kpts have been informed in cartesian coordinates.'
...
if orientation == 'landscape':
    ...
    if orientation != 'portrait':
        print 'Assuming portrait orientation.'
    fig_height_inches = fig_width_inches/aspect_ratio
...
if(args.icolormap != None):
    ...
    if(colormap_option<=len(colormaps)-1):
        ...
        print 'Colormap option ranges from 0 to ', len(colormaps)-1,'.'
        print 'Using the default colormap (gist_ncar).'
...
if manually_normalize_colorbar_min_and_maxval:
    print "Renormalizing color scale:"
    if(minval_for_colorbar != None):
        print "    Previous vmin = ", "%.*f" % (1, np.min(grid_freq)), ", new vmin = ", minval_for_colorbar
    else:
        minval_for_colorbar = np.min(grid_freq)
    if(maxval_for_colorbar != None):
        print "    Previous vmax = ", "%.*f" % (1, np.max(grid_freq)), ", new vmax = ", maxval_for_colorbar
    else:
...
if x_tiks_labels:
    print "x-ticks placed at ", x_tiks_positions
    print x_tiks_labels
...
if (args.save):
    ...
    if(fig_resolution[0].upper() == 'H'):
        print 'High-resolution figure (600 dpi) requested.'
        fig_resolution_in_dpi = 600
    elif (fig_resolution[0].upper() == 'M'):
        print 'Medium-resolution figure (300 dpi) requested.'
        fig_resolution_in_dpi = 300
    elif (fig_resolution[0].upper() == 'L'):
        print 'Low-resolution figure (100 dpi) requested.'
        fig_resolution_in_dpi = 100
    else:
        print 'Assuming medium-resolution (300 dpi) for the figure.'
        fig_resolution_in_dpi = 300
    print "Savig figure to file ",output_file_name," ..."
